...
struct wmdl_app_t : public w32_d3d9_chunky_app_t
{
	...
	{
		if (!grafxkid_tick(_guts._micron, _game))
			return false;
	...
	explicit wmdl_app_t()
		:w32_d3d9_chunky_app_t(GRAFXKID_SECONDS_PER_TICK, 0)
	{
	}
...
#if 1
	if (!__app.init("GrafxKid (c) 2025 nornware AB", true, ::GetSystemMetrics(SM_CXSCREEN) * 3 / 4, ::GetSystemMetrics(SM_CYSCREEN) * 3 / 4))
#else
	if (!__app.init("GrafxKid (c) 2025 nornware AB", false, 0, 0))
#endif
		return -1;

	//application-specific init
	if (!grafxkid_init(__app._guts._micron, __app._game))
		return -1;
	/*
