...
#define SM_REMOTESESSION  0x1000
	...
	static backbuffer_size_t __make(const D3DPRESENT_PARAMETERS& params)
	{
	...
	static void __revert(device_settings_t& settings, match_options_t& options)
	{
		const backbuffer_size_t size = settings.present_parameters.Windowed ? state.windowed_backbuffer_at_mode_change : state.fullscreen_backbuffer_at_mode_change;
	...
	//--------------------------------------------------------------------------------------
	static ::HRESULT __find_valid_resolution(const enum_device_settings_combo_t* aBestDeviceSettingsCombo, const D3DDISPLAYMODE aDisplayModeIn, D3DDISPLAYMODE* aBestDisplayMode)
	{
	...
	//--------------------------------------------------------------------------------------
	static enumeration_t* __prepare_enumeration_object(bool anEnumerate)
	{
		// create a new CD3DEnumeration object and enumerate all devices unless its already been done
		if (nullptr == state.m_d3d_enumeration)
		{
			state.m_d3d_enumeration = enumeration_t::instance();
		...
		if (anEnumerate)
		{
			...
			enumerate(*state.m_d3d_enumeration);
		}

		return state.m_d3d_enumeration;
	}
	...
	static void __update_device_stats(const ::D3DDEVTYPE aDeviceType, const ::DWORD someBehaviorFlags, const ::D3DADAPTER_IDENTIFIER9* anAdapterIdentifier)
	{
		...
		if (aDeviceType == D3DDEVTYPE_REF)
			::strcpy_s(state.m_device_stats, 256, "REF");
		else if (aDeviceType == D3DDEVTYPE_HAL)
			::strcpy_s(state.m_device_stats, 256, "HAL");
		else if (aDeviceType == D3DDEVTYPE_SW)
			::strcpy_s(state.m_device_stats, 256, "SW");
		...
		if (someBehaviorFlags & D3DCREATE_HARDWARE_VERTEXPROCESSING && someBehaviorFlags & D3DCREATE_PUREDEVICE)
		{
			if (aDeviceType == D3DDEVTYPE_HAL)
				::strcat_s(state.m_device_stats, 256, " (pure hw vp)");
			else
				::strcat_s(state.m_device_stats, 256, " (simulated pure hw vp)");
		}
		else if (someBehaviorFlags & D3DCREATE_HARDWARE_VERTEXPROCESSING)
		{
			if (aDeviceType == D3DDEVTYPE_HAL)
				::strcat_s(state.m_device_stats, 256, " (hw vp)");
			else
				::strcat_s(state.m_device_stats, 256, " (simulated hw vp)");
		}
		else if (someBehaviorFlags & D3DCREATE_MIXED_VERTEXPROCESSING)
		{
			if (aDeviceType == D3DDEVTYPE_HAL)
				::strcat_s(state.m_device_stats, 256, " (mixed vp)");
			else
				::strcat_s(state.m_device_stats, 256, " (simulated mixed vp)");
		}
		else if (someBehaviorFlags & D3DCREATE_SOFTWARE_VERTEXPROCESSING)
		{
			::strcat_s(state.m_device_stats, 256, " (sw vp)");
		}
		...
		if (aDeviceType == D3DDEVTYPE_HAL)
		{
			...
			::strcat_s(state.m_device_stats, 256, ": ");

			// Try to get a unique description from the CD3DEnumDeviceSettingsCombo
			device_settings_t* pDeviceSettings = state.m_device_settings;
			enumeration_t* pd3dEnum = __prepare_enumeration_object(false);
			const enum_device_settings_combo_t* pDeviceSettingsCombo = get_device_settings_combo(
			...
			{
				::strcat_s(state.m_device_stats, 256, pDeviceSettingsCombo->adapter_info->description);
			}
			else
			{
				::strcat_s(state.m_device_stats, 256, anAdapterIdentifier->Description);
			}
		}
	}
	...
	static void __display_error_message(::HRESULT aResult)
	{
		...
		if (bFound)
		{
			if (state.m_window_title[0] == 0)
				::MessageBoxA(hwnd(), strBuffer, "DirectX Application", MB_ICONERROR | MB_OK);
			else
				::MessageBoxA(hwnd(), strBuffer, state.m_window_title, MB_ICONERROR | MB_OK);
		}
	...
	static void __cleanup_3d_environment(const bool aReleaseSettings)
	{
		if (state.m_d3d_device != nullptr)
		{
			...
			{
				if (state.m_d3d_device->Release() > 0)
				{
					__display_error_message(DXUTERR_NONZEROREFCOUNT);
					DXUT_ERR("__cleanup_3d_environment", DXUTERR_NONZEROREFCOUNT);
				}
			}
			state.m_d3d_device = nullptr;

			if (aReleaseSettings)
			{
				device_settings_t* oldDeviceSettings = state.m_device_settings;
				SAFE_DELETE(oldDeviceSettings);
				state.m_device_settings = nullptr;
			}

			::ZeroMemory(&state.m_backbuffer_surface_desc, sizeof(::D3DSURFACE_DESC));

			::ZeroMemory(&state.m_caps, sizeof(::D3DCAPS9));

			state.m_device.created = false;
		}
	}
	...
	static ::HRESULT __get_adapter_ordinal_from_monitor(::HMONITOR aMonitor, ::UINT* anAdapterOrdinal)
	{
		*anAdapterOrdinal = 0;
		...
		const enumeration_t* ENUMERATION = __prepare_enumeration_object(false);

		const std::vector<enum_adapter_info_t*>* ADAPTER_LIST = &ENUMERATION->adapter_info_list;
		for (uint32_t iAdapter = 0; iAdapter < ADAPTER_LIST->size(); ++iAdapter)
		{
			const enum_adapter_info_t* ADAPTER_INFO = (*ADAPTER_LIST)[iAdapter];
			const HMONITOR hAdapterMonitor = state.m_d3d->GetAdapterMonitor(ADAPTER_INFO->adapter_ordinal);
	...
	{
		//state.m_allow_shortcut_keys = anAllowKeys;

		if (anAllowKeys)
		{
			// Restore StickyKeys/etc to original state and enable Windows key      
			::SystemParametersInfo(SPI_SETSTICKYKEYS, sizeof(STICKYKEYS), &state.m_startup_keys.sticky, 0);
			::SystemParametersInfo(SPI_SETTOGGLEKEYS, sizeof(TOGGLEKEYS), &state.m_startup_keys.toggle, 0);
			::SystemParametersInfo(SPI_SETFILTERKEYS, sizeof(FILTERKEYS), &state.m_startup_keys.filter, 0);
		}
		else
		{
			...
			STICKYKEYS skOff = state.m_startup_keys.sticky;
			...
			TOGGLEKEYS tkOff = state.m_startup_keys.toggle;
			...
			FILTERKEYS fkOff = state.m_startup_keys.filter;
	...
	static void __update_back_buffer_desc()
	{
		...
		hr = state.m_d3d_device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
		::ZeroMemory(&state.m_backbuffer_surface_desc, sizeof(D3DSURFACE_DESC));
		if (SUCCEEDED(hr))
		{
			pBackBuffer->GetDesc(&state.m_backbuffer_surface_desc);
	...
	//--------------------------------------------------------------------------------------
	static void __build_valid_device_settings(device_settings_t* someValidDeviceSettings, const enum_device_settings_combo_t* aBestDeviceSettingsCombo, const device_settings_t* someDeviceSettingsIn, const match_options_t* someMatchOptions)
	{
		::D3DDISPLAYMODE adapterDesktopDisplayMode;
		state.m_d3d->GetAdapterDisplayMode(aBestDeviceSettingsCombo->adapter_ordinal, &adapterDesktopDisplayMode);
		...
		::ZeroMemory(someValidDeviceSettings, sizeof(device_settings_t));
		...
		someValidDeviceSettings->present_parameters.hDeviceWindow = aBestDeviceSettingsCombo->windowed ? state.m_hwnd.device_windowed : state.m_hwnd.device_fullscreen;
	...
	//--------------------------------------------------------------------------------------
	static float __rank_device_combo(const enum_device_settings_combo_t* aDeviceSettingsCombo, const device_settings_t* someOptimalDeviceSettings, const ::D3DDISPLAYMODE* anAdapterDesktopDisplayMode)
	{
	...
	//--------------------------------------------------------------------------------------
	static bool __does_device_combo_match_preserve_options(const enum_device_settings_combo_t* aDeviceSettingsCombo, const device_settings_t* someDeviceSettingsIn, const match_options_t* someMatchOptions)
	{
		//---------------------
		// Adapter ordinal
		//---------------------
	...
	// into account.
	//--------------------------------------------------------------------------------------
	static void __build_optimal_device_settings(device_settings_t* someOptimalDeviceSettings, const device_settings_t* someDeviceSettingsIn, const match_options_t* someMatchOptions)
	{
		::D3DDISPLAYMODE adapterDesktopDisplayMode;

		::ZeroMemory(someOptimalDeviceSettings, sizeof(device_settings_t));
		...
		if (someMatchOptions->adapter_format == match_type_t::IGNORE_INPUT)
		{
			...
			state.m_d3d->GetAdapterDisplayMode(someOptimalDeviceSettings->adapter_ordinal, &adapterDesktopDisplayMode);
	...
	//--------------------------------------------------------------------------------------
	static HRESULT __find_valid_device_settings(device_settings_t* someSettingsOut, const device_settings_t* someSettingsIn, const match_options_t* someMatchOptions)
	{
		...
		// The default value may not exist on the system, but later this will be taken 
		// into account.
		device_settings_t optimalDeviceSettings;
		...
		const enum_device_settings_combo_t* BEST_DEVICE_SETTINGS_COMBO = nullptr;
		::D3DDISPLAYMODE adapterDesktopDisplayMode;
		...
		for (uint32_t iAdapter = 0; iAdapter < ADAPTER_LIST->size(); ++iAdapter)
		{
			...
			state.m_d3d->GetAdapterDisplayMode(ADAPTER_INFO->adapter_ordinal, &adapterDesktopDisplayMode);
			state.desktop_resolution = { adapterDesktopDisplayMode.Width, adapterDesktopDisplayMode.Height };
			...
			{
				const enum_device_info_t* DEVICE_INFO = ADAPTER_INFO->device_info_list[iDeviceInfo];
				...
				{
					const enum_device_settings_combo_t* DEVICE_SETTINGS_COMBO = DEVICE_INFO->device_settings_combo_list[iDeviceCombo];
		...
		device_settings_t validDeviceSettings;
	...
	static void __setup_cursor()
	{
		...
		if (!is_windowed() && state.m_d3d_device)
		{
			if (state.m_show_cursor_when_fullscreen)
			{
				::SetCursor(::LoadCursor(nullptr, IDC_ARROW));

				state.m_d3d_device->ShowCursor(true);
			}
			else
			{
				::SetCursor(nullptr); // Turn off Windows cursor in full screen mode
				state.m_d3d_device->ShowCursor(false);
			}
		}

		// Clip cursor if requested
		if (!is_windowed() && state.m_clip_cursor_when_fullscreen)
		{
			// Confine cursor to full screen window
			::RECT rcWindow;
			::GetWindowRect(state.m_hwnd.device_fullscreen, &rcWindow);
			::ClipCursor(&rcWindow);
		}
		else
		{
	...
	static ::HRESULT __reset_3d_environment()
	{
		...
		assert(state.m_d3d_device != nullptr);
		...
		// reset the device
		device_settings_t* deviceSettings = state.m_device_settings;
		hr = state.m_d3d_device->Reset(&deviceSettings->present_parameters);
		if (FAILED(hr))
		{
		...
		// Success
		state.m_device_objects.reset = true;
	...
	static ::HRESULT __create_3d_environment(::IDirect3DDevice9* aDeviceFromApp)
	{
		...
		assert(nullptr == state.m_d3d_device);
		device_settings_t* newDeviceSettings = state.m_device_settings;
		...
		else
		{
			...
			state.m_d3d_device = aDeviceFromApp;
		}
		...
		if (newDeviceSettings->device_type == D3DDEVTYPE_REF && state.m_exit_code == 0)
			state.m_exit_code = 11;
		else if (newDeviceSettings->device_type == D3DDEVTYPE_HAL && state.m_exit_code == 11)
			state.m_exit_code = 0;
		...
		state.m_d3d_device->GetDeviceCaps(&state.m_caps);

		// update the device stats text
		enumeration_t* enumeration = __prepare_enumeration_object(false);
		const enum_adapter_info_t* ADAPTER_INFO = get_adapter_info(newDeviceSettings->adapter_ordinal, *enumeration);
		...
		state.m_device_objects.created = true;
	...
	//--------------------------------------------------------------------------------------
	static device_settings_t __get_device_settings()
	{
		const device_settings_t* DEVICE_SETTINGS = state.m_device_settings;
		if (DEVICE_SETTINGS)
			return *DEVICE_SETTINGS;

		device_settings_t ds;
		::ZeroMemory(&ds, sizeof(device_settings_t));
		return ds;
	}
	...
	//--------------------------------------------------------------------------------------
	static ::HRESULT __change_device(device_settings_t* pNewDeviceSettings, ::IDirect3DDevice9* aDeviceFromApp, bool aForceRecreate, bool aClipWindowToSingleAdapter)
	{
		/*
		{
		...
		*/
		...
		if (state.m_d3d == nullptr)
			return S_FALSE;

		// Make a copy of the pNewDeviceSettings on the heap
		device_settings_t* newDeviceSettingsOnHeap = new device_settings_t;
		if (newDeviceSettingsOnHeap == nullptr)
			return E_OUTOFMEMORY;
		::memcpy(newDeviceSettingsOnHeap, pNewDeviceSettings, sizeof(device_settings_t));
		...
		if (state.app)
		{
			...
			const bool bContinue = state.app->modify_device_settings(caps, *pNewDeviceSettings);
			if (!bContinue)
			{
		...
		state.m_device_settings = pNewDeviceSettings;

		pause(__FUNCTION__, "starting", true, true);
		...
		state.m_ignore_size_change = true;
		...
		if (pNewDeviceSettings->present_parameters.Windowed)
		{
			...
			if (oldDeviceSettings && !oldDeviceSettings->present_parameters.Windowed)
			{
				...
				state.fullscreen_backbuffer_at_mode_change = __make(oldDeviceSettings->present_parameters);

				// Restore windowed mode style
				::SetWindowLong(state.m_hwnd.device_windowed, GWL_STYLE, state.m_windowed_style_at_mode_change);
			}
			...
			if (state.m_hwnd.device_fullscreen != state.m_hwnd.device_windowed)
				::ShowWindow(state.m_hwnd.device_fullscreen, SW_HIDE);

			// If using the same window for windowed and fullscreen mode, reattach menu if one exists
			if (state.m_hwnd.device_fullscreen == state.m_hwnd.device_windowed)
			{
				if (state.m_menu != nullptr)
					::SetMenu(state.m_hwnd.device_windowed, state.m_menu);
			}
		}
		else
		{
			...
			if (oldDeviceSettings == nullptr || (oldDeviceSettings && oldDeviceSettings->present_parameters.Windowed))
			{
				...
				::WINDOWPLACEMENT* pwp = &state.m_windowed_placement;
				::ZeroMemory(pwp, sizeof(::WINDOWPLACEMENT));
				pwp->length = sizeof(::WINDOWPLACEMENT);
				::GetWindowPlacement(state.m_hwnd.device_windowed, pwp);
				state.m_topmost_while_windowed = ((::GetWindowLong(state.m_hwnd.device_windowed, GWL_EXSTYLE) & WS_EX_TOPMOST) != 0);
				DWORD dwStyle = ::GetWindowLong(state.m_hwnd.device_windowed, GWL_STYLE);
				dwStyle &= ~WS_MAXIMIZE & ~WS_MINIMIZE; // remove minimize/maximize style
				state.m_windowed_style_at_mode_change = dwStyle;
				if (oldDeviceSettings)
					state.windowed_backbuffer_at_mode_change = __make(oldDeviceSettings->present_parameters);
			}
			...
			// set FS window style
			::SetWindowLong(state.m_hwnd.device_fullscreen, GWL_STYLE, WS_POPUP | WS_SYSMENU);

			// If using the same window for windowed and fullscreen mode, save and remove menu 
			if (state.m_hwnd.device_fullscreen == state.m_hwnd.device_windowed)
			{
				::HMENU hMenu = ::GetMenu(state.m_hwnd.device_fullscreen);
				state.m_menu = hMenu;
				::SetMenu(state.m_hwnd.device_fullscreen, nullptr);
			}

			::WINDOWPLACEMENT wpFullscreen;
			::ZeroMemory(&wpFullscreen, sizeof(::WINDOWPLACEMENT));
			wpFullscreen.length = sizeof(::WINDOWPLACEMENT);
			::GetWindowPlacement(state.m_hwnd.device_fullscreen, &wpFullscreen);
			if ((wpFullscreen.flags & WPF_RESTORETOMAXIMIZED) != 0)
			{
				...
				::SetWindowPlacement(state.m_hwnd.device_fullscreen, &wpFullscreen);
			}
		}
		...
		if (!aForceRecreate &&
			(aDeviceFromApp == nullptr || aDeviceFromApp == state.m_d3d_device) &&
			state.m_d3d_device &&
			oldDeviceSettings &&
			...
			oldDeviceSettings->behavior_flags == pNewDeviceSettings->behavior_flags)
		{
			...
			if (FAILED(hr))
			{
				...
				else // DXUTERR_RESETTINGDEVICE
				{
					...
					state.m_device_settings = oldDeviceSettings;
		...
		else
		{
			...
			if (FAILED(hr))
			{
				...
				__display_error_message(hr);
				pause(__FUNCTION__, "__create3DEnvironment() FAILED", false, false);
				state.m_ignore_size_change = false;
				return hr;
			}
		}
		...
		state.m_adapter_monitor = state.m_d3d->GetAdapterMonitor(pNewDeviceSettings->adapter_ordinal);
		...
		if (oldDeviceSettings && !oldDeviceSettings->present_parameters.Windowed && pNewDeviceSettings->present_parameters.Windowed)
		{
			...
			::WINDOWPLACEMENT* pwp = &state.m_windowed_placement;
			::SetWindowPlacement(state.m_hwnd.device_windowed, pwp);

			// Also restore the z-order of window to previous state
			::HWND hWndInsertAfter = state.m_topmost_while_windowed ? HWND_TOPMOST : HWND_NOTOPMOST;
			::SetWindowPos(state.m_hwnd.device_windowed, hWndInsertAfter, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOREDRAW | SWP_NOSIZE);
		}
			...
			!bKeepCurrentWindowSize)          // only resize if myPresentParameters.BackbufferWidth/Height were not 0
		{
			...
			if (::IsIconic(state.m_hwnd.device_windowed))
			{
				...
				::GetWindowPlacement(state.m_hwnd.device_windowed, &wp);
				...
				if ((wp.flags & WPF_RESTORETOMAXIMIZED) != 0 && wp.showCmd == SW_SHOWMINIMIZED)
				{
					...
					::ShowWindow(state.m_hwnd.device_windowed, SW_RESTORE);
					::RECT rcClient;
					::GetClientRect(state.m_hwnd.device_windowed, &rcClient);
					nClientWidth = (UINT)(rcClient.right - rcClient.left);
					nClientHeight = (UINT)(rcClient.bottom - rcClient.top);
					::ShowWindow(state.m_hwnd.device_windowed, SW_MINIMIZE);
				}
				else
				{
					...
					RECT rcFrame = { 0 };
					::AdjustWindowRect(&rcFrame, state.m_windowed_style_at_mode_change, state.m_menu != nullptr);
			...
			if (aClipWindowToSingleAdapter && !::IsIconic(state.m_hwnd.device_windowed))
			{
				// get the rect of the monitor attached to the adapter
				MONITORINFO miAdapter;
				miAdapter.cbSize = sizeof(MONITORINFO);
				::HMONITOR hAdapterMonitor = state.m_d3d->GetAdapterMonitor(pNewDeviceSettings->adapter_ordinal);
				get_monitor_info(hAdapterMonitor, &miAdapter);
				::HMONITOR hWindowMonitor = monitor_from_window(hwnd(), MONITOR_DEFAULTTOPRIMARY);

				// get the rect of the window
				::RECT rcWindow;
				::GetWindowRect(state.m_hwnd.device_windowed, &rcWindow);
				...
				{
					if (hWindowMonitor == hAdapterMonitor && ::IsZoomed(state.m_hwnd.device_windowed))
					{
		...
		if (bNeedToResize)
		{
			...
			if (::IsIconic(state.m_hwnd.device_windowed))
				::ShowWindow(state.m_hwnd.device_windowed, SW_RESTORE);
			if (::IsZoomed(state.m_hwnd.device_windowed)) // doing the IsIconic() check first also handles the WPF_RESTORETOMAXIMIZED case
				::ShowWindow(state.m_hwnd.device_windowed, SW_RESTORE);
			...
			if (aClipWindowToSingleAdapter)
			{
				...
				MONITORINFO miAdapter;
				miAdapter.cbSize = sizeof(MONITORINFO);
				get_monitor_info(state.m_d3d->GetAdapterMonitor(pNewDeviceSettings->adapter_ordinal), &miAdapter);

				// get the rect of the monitor attached to the window
				MONITORINFO miWindow;
				miWindow.cbSize = sizeof(MONITORINFO);
				get_monitor_info(monitor_from_window(hwnd(), MONITOR_DEFAULTTOPRIMARY), &miWindow);
				...
				::AdjustWindowRect(&rcResizedWindow, ::GetWindowLong(state.m_hwnd.device_windowed, GWL_STYLE), state.m_menu != nullptr);
				...
				// after resetting the device rather than beforehand to ensure 
				// that the monitor resolution is correct and does not limit the size of the new window.
				::SetWindowPos(state.m_hwnd.device_windowed, 0, rcResizedWindow.left, rcResizedWindow.top, nWindowWidth, nWindowHeight, SWP_NOZORDER);
			}
			else
			{
				// Make a window rect with a client rect that is the same size as the backbuffer
				...
				::AdjustWindowRect(&rcWindow, ::GetWindowLong(state.m_hwnd.device_windowed, GWL_STYLE), state.m_menu != nullptr);
				...
				::SetWindowPos(state.m_hwnd.device_windowed, 0, 0, 0, cx, cy, SWP_NOZORDER | SWP_NOMOVE);
			}
			...
			if (nClientWidth != pNewDeviceSettings->present_parameters.BackBufferWidth || nClientHeight != pNewDeviceSettings->present_parameters.BackBufferHeight)
			{
				...
				device_settings_t deviceSettings(__get_device_settings());
				...
				if (FAILED(hr))
				{
					SAFE_DELETE(oldDeviceSettings);
					__cleanup_3d_environment(true);
					pause(__FUNCTION__, "__changeDevice() FAILED", false, false);
					state.m_ignore_size_change = false;
					return hr;
				}
			}
		...
		SAFE_DELETE(oldDeviceSettings);
		state.m_ignore_size_change = false;
		pause(__FUNCTION__, "finished", false, false);
		state.m_device.created = true;
	...
	static void __check_for_window_size_change()
	{
		...
		if (state.m_ignore_size_change ||
			!state.m_device.created ||
			!state.m_device_settings->present_parameters.Windowed)
			return;

		::RECT rcCurrentClient;
		::GetClientRect(hwnd(), &rcCurrentClient);

		if ((UINT)rcCurrentClient.right != state.m_device_settings->present_parameters.BackBufferWidth ||
			(UINT)rcCurrentClient.bottom != state.m_device_settings->present_parameters.BackBufferHeight)
		{
	...
	static void __check_for_window_changing_monitors()
	{
		...
		if (!state.m_auto_change_adapter || state.m_ignore_size_change || !state.m_device.created || !state.m_device_settings->present_parameters.Windowed)
			return;

		::HRESULT hr;
		HMONITOR hWindowMonitor = monitor_from_window(hwnd(), MONITOR_DEFAULTTOPRIMARY);
		HMONITOR hAdapterMonitor = state.m_adapter_monitor;
			...
			if (SUCCEEDED(__get_adapter_ordinal_from_monitor(hWindowMonitor, &newOrdinal)))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					if (hr == E_ABORT)
					{
						...
						state.m_auto_change_adapter = false;
					}
					else if (FAILED(hr))
					{
						shutdown();
	...
	static ::LRESULT CALLBACK __static_wnd_proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if (state.app)
		{
			...
			LRESULT nResult = state.app->msg_proc(hWnd, uMsg, wParam, lParam, &bNoFurtherProcessing);
		...
		case WM_PAINT:
		{
			...
			if (state.m_d3d_device && is_rendering_paused() && state.m_device_objects.created && state.m_device_objects.reset)
			{
				::HRESULT hr;

				if (state.app)
					state.app->frame_render(state.m_time, state.m_elapsed_time);
		...
		case WM_SIZE:
			if (SIZE_MINIMIZED == wParam)
			{
				...
				{
					pause(__FUNCTION__, "WM_SIZE == SIZE_MINIMIZED, no app", true, true); // pause while we're minimized
				}

				state.m_minimized = true;
				state.m_maximized = false;
			}
			else
			{
				RECT rcCurrentClient;
				...
				{
					if (state.m_minimized)
						pause(__FUNCTION__, "WM_SIZE == SIZE_MAXIMIZED", false, false); // Unpause since we're no longer minimized
					state.m_minimized = false;
					state.m_maximized = true;
					__check_for_window_size_change();
					__check_for_window_changing_monitors();
				}
				else if (SIZE_RESTORED == wParam)
				{
					if (state.m_maximized)
					{
						state.m_maximized = false;
						__check_for_window_size_change();
						__check_for_window_changing_monitors();
					}
					else if (state.m_minimized)
					{
						pause(__FUNCTION__, "WM_SIZE == SIZE_RESTORED", false, false); // Unpause since we're no longer minimized
						state.m_minimized = false;
						__check_for_window_size_change();
						__check_for_window_changing_monitors();
					}
					else if (state.m_inside.size_move)
					{
		...
		case WM_ENTERSIZEMOVE:
			...
			pause(__FUNCTION__, "WM_ENTERSIZEMOVE", true, true);
			state.m_inside.size_move = true;
			break;

		case WM_EXITSIZEMOVE:
			pause(__FUNCTION__, "WM_EXITSIZEMOVE", false, false);
			__check_for_window_size_change();
			__check_for_window_changing_monitors();
			state.m_inside.size_move = false;
			break;

		case WM_MOUSEMOVE:
			if (state.m_active && !is_windowed())
			{
				if (state.m_d3d_device)
				{
					::POINT ptCursor;
					::GetCursorPos(&ptCursor);
					state.m_d3d_device->SetCursorPosition(ptCursor.x, ptCursor.y, 0);
				}
			}
			break;
		...
		case WM_SETCURSOR:
			if (state.m_active && !is_windowed())
			{
				if (state.m_d3d_device && state.m_show_cursor_when_fullscreen)
					state.m_d3d_device->ShowCursor(true);
		...
		case WM_ACTIVATEAPP:
			if (wParam == TRUE && !state.m_active) // Handle only if previously not active 
			{
				state.m_active = true;
				...
				if (state.m_minimized_while_fullscreen)
				{
					pause(__FUNCTION__, "WM_ACTIVATEAPP == TRUE", false, false); // Unpause since we're no longer minimized
					state.m_minimized_while_fullscreen = false;
				}

				// Upon returning to this app, potentially disable shortcut keys 
				// (Windows key, accessibility shortcuts) 
				__allow_shortcut_keys(is_windowed() ? true : false);

			}
			else if (wParam == FALSE && state.m_active) // Handle only if previously active 
			{
				...
				if (!is_windowed())
				{
					...
					state.m_minimized_while_fullscreen = true;
				}
		...
		case WM_ENTERMENULOOP:
			...
			pause(__FUNCTION__, "WM_ENTERMENULOOP", true, true);
			break;

		case WM_EXITMENULOOP:
			pause(__FUNCTION__, "WM_EXITMENULOOP", false, false);
			break;
...
#define PBT_APMRESUMESUSPEND 0x0007
		...
		case WM_SYSKEYDOWN:
		{
			...
			case VK_RETURN:
			{
				...
				if ((lParam & dwMask) != 0) // Alt is down also
				{
					...
					pause(__FUNCTION__, "WM_SYSKEYDOWN, state.getHandleAltEnter() == true, PRE-toggle_fullscreen()", true, true);
					toggle_fullscreen();
					pause(__FUNCTION__, "WM_SYSKEYDOWN, state.getHandleAltEnter() == true, POST-toggle_fullscreen()", false, false);
					return 0;
				}
			}
			}
		...
		case WM_KEYDOWN:
		{
			//state.m_handle_default_hotkeys;
			break;
		}
		...
		if (!true || state.m_menu == nullptr && (uMsg == WM_SYSKEYDOWN || uMsg == WM_SYSKEYUP) && wParam == VK_F10)
			return 0;
	...
	::HRESULT init()
	{
		...
		state.m_startup_keys.sticky.cbSize = sizeof(::STICKYKEYS);
		::SystemParametersInfo(SPI_GETSTICKYKEYS, sizeof(::STICKYKEYS), &state.m_startup_keys.sticky, 0);

		state.m_startup_keys.toggle.cbSize = sizeof(::TOGGLEKEYS);
		::SystemParametersInfo(SPI_GETTOGGLEKEYS, sizeof(::TOGGLEKEYS), &state.m_startup_keys.toggle, 0);

		state.m_startup_keys.filter.cbSize = sizeof(::FILTERKEYS);
		SystemParametersInfo(SPI_GETFILTERKEYS, sizeof(::FILTERKEYS), &state.m_startup_keys.filter, 0);
		...
		//state.m_show_msg_box_on_error = true;
		//state.m_handle_default_hotkeys = true;
		//state.m_handle_alt_enter = true;
		...
		if (nullptr == state.m_d3d)
		{
			...
			if (::GetSystemDirectory(systemPath, sizeof(systemPath)))
			{
				...
				if (library)
				{
					...
					if (function)
						state.m_d3d = function(D3D_SDK_VERSION);
					//else
	...
	::HRESULT create_window(const char* aWindowTitle, const ::DWORD aWindowStyle)
	{
		...
		if (state.m_hwnd.focus == nullptr)
		{
			...
			//if (state.m_override_start_x != -1)
			//	x = state.m_override_start_x;
			//if (state.m_override_start_y != -1)
			//	y = state.m_override_start_y;

			//state.m_window_created_with_default_positions = false;
			//if (x == CW_USEDEFAULT && y == CW_USEDEFAULT)
			//	state.m_window_created_with_default_positions = true;

			// find the window's initial size, but it might be changed later
			//int32_t nDefaultWidth = 640;
			//int32_t nDefaultHeight = 480;
			//if (state.m_override_width != 0)
			//	nDefaultWidth = state.m_override_width;
			//if (state.m_override_height != 0)
			//	nDefaultHeight = state.m_override_height;
			::RECT rc{ 0, 0, 640, 480 };

			::AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW, false);	//no menu

			::strcpy_s(state.m_window_title, 256, aWindowTitle);
			...
			state.m_window.created = true;
			state.m_hwnd.focus = hWnd;
			state.m_hwnd.device_fullscreen = hWnd;
			state.m_hwnd.device_windowed = hWnd;
		}
	...
	//--------------------------------------------------------------------------------------
	::HRESULT create_device(UINT anAdapterOrdinal, bool windowed, int32_t aSuggestedWidth, int32_t aSuggestedHeight, app_i* anApp)
	{
		::HRESULT hr;
		...
		*/
		...
		::ZeroMemory(&deviceSettings, sizeof(device_settings_t));
		...
		//if (state.m_override_width != 0)
		//	deviceSettings.present_parameters.BackBufferWidth = state.m_override_width;
		//if (state.m_override_height != 0)
		//	deviceSettings.present_parameters.BackBufferHeight = state.m_override_height;

		//if (state.m_override_adapter_ordinal != -1)
		//	deviceSettings.adapter_ordinal = state.m_override_adapter_ordinal;

		/*
		if (state.m_override_fullscreen)
		{
			deviceSettings.present_parameters.Windowed = FALSE;
			if (state.getOverrideWidth() == 0 && state.getOverrideHeight() == 0)
				matchOptions.resolution = match_type_t::IGNORE_INPUT;
		}
		*/
		//if (state.m_override_windowed)
		//	deviceSettings.present_parameters.Windowed = TRUE;

		/*
		if (state.m_override_force_hal)
		{
			deviceSettings.device_type = D3DDEVTYPE_HAL;
			matchOptions.device_type = match_type_t::PRESERVE_INPUT;
		}
		if (state.m_override_force_ref)
		{
			deviceSettings.device_type = D3DDEVTYPE_REF;
			matchOptions.device_type = match_type_t::PRESERVE_INPUT;
		}

		if (state.m_override_force_pure_hwvp)
		{
		...
		else if (state.getOverrideForceVsync() == 1)
		{
	...
	//--------------------------------------------------------------------------------------
	::HRESULT toggle_fullscreen(const bool aDoToggle)
	{
		::HRESULT hr;
	...
	//--------------------------------------------------------------------------------------
	void pause(const char* /*aFunction*/, const char* /*aContext*/, const bool aPauseTime, const bool aPauseRendering)
	{
		//FS_LOG("aFunction = %s, aContext = %s, aPauseTime = %u, aPauseRendering = %u", aFunction, aContext, aPauseTime, aPauseRendering);

		//int32_t nPauseTimeCount = state.m_pause_time_count;
		state.m_pause_time_count += (aPauseTime ? +1 : -1);
		if (state.m_pause_time_count < 0)
			state.m_pause_time_count = 0;
		//state.m_pause_time_count = nPauseTimeCount;

		//int32_t nPauseRenderingCount = state.m_pause_rendering_count;
		state.m_pause_rendering_count += (aPauseRendering ? +1 : -1);
		if (state.m_pause_rendering_count < 0)
			state.m_pause_rendering_count = 0;
		//state.m_pause_rendering_count = nPauseRenderingCount;
		...
		//state.m_time_paused = nPauseTimeCount > 0;
	}
	...
	static void __do_frame(::IDirect3DDevice9* device, const float now, const float elapsed)
	{
		...
		if (state.app)
		{
			state.app->frame_move(now, elapsed);
		...
		if (!is_rendering_paused())
		{
			...
			if (state.app)
			{
				shouldPresent = state.app->frame_render(now, elapsed);
				if (nullptr == state.m_d3d_device)// Handle shutdown from inside callback
					return;
			}
			...
			{
				::GetClientRect(hwnd(), &rcClient);
				assert(state.m_backbuffer_surface_desc.Width == (UINT)rcClient.right);
				assert(state.m_backbuffer_surface_desc.Height == (UINT)rcClient.bottom);
			}
#endif
	...
	//      - Calling Present()
	//--------------------------------------------------------------------------------------
	static void __do_idle_time(app_i* anApp)
	{
		::HRESULT hr;
		...
		if (state.m_device_lost || is_rendering_paused() || !state.m_active)
		{
			...
			if (!anApp || !anApp->is_fixed_tick_rate())
			{
			...
			else
			{
				...
				const uint32_t BGSM = anApp->fixed_tick_background_sleep_milliseconds();
				if (BGSM)
					::Sleep(BGSM);
			}
		}

		if (nullptr == state.m_d3d_device)
		{
			if (state.m_device_lost)
			{
				device_settings_t deviceSettings(__get_device_settings());
		...
		if (state.m_device_lost && !is_rendering_paused())
		{
			// Test the cooperative level to see if it's okay to render
			if (FAILED(hr = state.m_d3d_device->TestCooperativeLevel()))
			{
				if (D3DERR_DEVICELOST == hr)
				{
				...
				if (is_windowed())
				{
					::D3DDISPLAYMODE adapterDesktopDisplayMode;
					const device_settings_t* DEVICE_SETTINGS = state.m_device_settings;
					state.m_d3d->GetAdapterDisplayMode(DEVICE_SETTINGS->adapter_ordinal, &adapterDesktopDisplayMode);
				...
				if (FAILED(hr = __reset_3d_environment()))
				{
					...
					else
					{
						...
						// so recreate the device to try to recover
						if (FAILED(__change_device(state.m_device_settings, nullptr, true, false)))
						{
							shutdown();
							return;
						}
		...
		//fixed update central
		{
			...
			if (anApp && anApp->is_fixed_tick_rate())
			{
				...
				const float FIXED_ELAPSED = anApp->seconds_per_fixed_tick();
				...
				if (TIME_VALUES.elapsed < FIXED_ELAPSED)
				{
					...
					if (elapsed > FIXED_ELAPSED)
					{
						...
						while (elapsed > FIXED_ELAPSED)
						{
							...
							__do_frame(state.m_d3d_device, (float)now, FIXED_ELAPSED);
							now += FIXED_ELAPSED;
					...
					{
						anApp->do_off_tick_processing(now);
					}
				}
				else
				{
			...
			{
				//do frame
				__do_frame(state.m_d3d_device, (float)TIME_VALUES.time, TIME_VALUES.elapsed);

				//do "off tick" processing also
				if (anApp)
					anApp->do_off_tick_processing(TIME_VALUES.time);
			}
		}
		//fixed update central
		//fixed update central
		...
		{
			++state.m_current_frame_number;

			// Check to see if the app should shutdown due to cmdline
			if (0 != state.getOverrideQuitAfterFrame())
			{
				if (state.m_current_frame_number > state.getOverrideQuitAfterFrame())
					shutdown();
			}
		}
		*/
	...
	::HRESULT main_loop(app_i* anApp)
	{
		...
		if (!state.m_init.created || !state.m_window.created || !state.m_device.created)
		{
			if ((state.m_exit_code == 0) || (state.m_exit_code == 11))
				state.m_exit_code = 1;
			return DXUT_ERR_MSGBOX("MainLoop", E_FAIL);
		...
		state.m_inside.main_loop = false;
	...
	//--------------------------------------------------------------------------------------
	void shutdown(const int32_t nExitCode)
	{
		HWND hWnd = hwnd();
		if (hWnd != nullptr)
			::SendMessage(hWnd, WM_CLOSE, 0, 0);

		state.m_exit_code = nExitCode;
		...
		state.m_d3d_enumeration = nullptr;

		SAFE_RELEASE(state.m_d3d);
		state.m_d3d = nullptr;
	}

	void set_cursor_settings(const bool bShowCursorWhenFullScreen, const bool bClipCursorWhenFullScreen)
	{
		state.m_clip_cursor_when_fullscreen = bClipCursorWhenFullScreen;
		state.m_show_cursor_when_fullscreen = bShowCursorWhenFullScreen;
		__setup_cursor();
	}
}