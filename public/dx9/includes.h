...
#if defined(DEBUG) || defined(_DEBUG)
	#ifndef VERIFY
		#define VERIFY(x)           { hr = x; if( FAILED(hr) ) { dx9::trace( __FILE__, (DWORD)__LINE__, hr, #x, true ); } }
	#endif
	#ifndef VERIFY_RETURN
		#define VERIFY_RETURN(x)    { hr = x; if( FAILED(hr) ) { return dx9::trace( __FILE__, (DWORD)__LINE__, hr, #x, true ); } }
	#endif
#else
	#ifndef VERIFY
