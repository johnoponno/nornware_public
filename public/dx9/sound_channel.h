...
namespace sound
{
	...
	bool buffer_init(
		...
		::IDirectSound& aDirectSound, buffer_t& b);
	...
	bool channel_stop(const void* aHandle,
		channel_t& c);

	bool channel_play(const bool aLooped, const float aVolume, const float aPan, const float aFrequency, const void* aHandle,
		channel_t& c);
}
