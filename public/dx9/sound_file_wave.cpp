...
namespace sound
{
	static void __destroy(file_wave_t& fws)
	{
		//clear header
	...
	static bool __parse_wave_data(const uint8_t* aFileData, file_wave_t& fws)
	{
	...
	bool file_wave_load(const char* aFileName, file_wave_t& fws)
	{
		...
		//parse wave data
		const bool result = __parse_wave_data((uint8_t*)contents.data, fws);
