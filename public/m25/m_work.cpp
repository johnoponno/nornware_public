...
namespace m25
{
		...
		c_xorshift128_t& out_prng, m_mob_t& out_mob)
	{
		...
		out_mob.swimmer.next_vector_time = in_tick * M_SECONDS_PER_TICK + out_prng.float32(M_SWIMMER_MIN_NEXT_VECTOR_TIME, M_SWIMMER_MAX_NEXT_VECTOR_TIME);
	}
		...
		for (m_mob_t& mob : out_mu.mobs)
		{
			...
			case M_LOGIC_WALKER2:
			{
				...
				else
				{
...
#if 0
					__walker_collide(im, mu, OP, { 0.f, M_TILE_ASPECT * 2.f }, M_TYPE_SOLID, mob);
#else
					switch (m_tile_type_at_position(mob.position + c_vec2f_t{ 0.f, M_TILE_ASPECT * 2.f }, true, in_im, out_mu))
					{
					default:
						mob.position = OP;
					...
					//check sides
					if (mob.walker.speed < 0.f)
						__walker_collide(in_im, out_mu, OP, { -8.f, M_TILE_ASPECT }, M_TYPE_AIR, mob);
					else
						__walker_collide(in_im, out_mu, OP, { 8.f, M_TILE_ASPECT }, M_TYPE_AIR, mob);
				}
			}
			...
			case M_LOGIC_JUMPER2:
				if (mob.jumper.start_time < in_tick * M_SECONDS_PER_TICK)
				{
					...
					if (mob.position.y > mob.jumper.start)
					{
						...
						mob.jumper.speed = M_JUMPER_SPEED;
					}
			...
			case M_LOGIC_SWIMMER2:
			{
				...
				mob.swimmer.speed += mob.swimmer.vector * (M_SWIMMER_ACC * M_SECONDS_PER_TICK);
				mob.swimmer.speed -= mob.swimmer.speed * (M_SWIMMER_BRAKE * M_SECONDS_PER_TICK);
	...
	{
		return{ (int32_t)(in_world.x / M_TILE_ASPECT), (int32_t)(in_world.y / M_TILE_ASPECT) };
	}

	uint32_t m_grid_to_offset(const c_vec2i_t& in_grid)
	{
		return in_grid.x + in_grid.y * M_WORLD_WIDTH;
	}
	...
	c_vec2f_t m_offset_to_world(const uint32_t in_offset)
	{
		...
		wp.x = (float)((in_offset % M_WORLD_WIDTH) * M_TILE_ASPECT + M_TILE_ASPECT / 2);
		wp.y = (float)((in_offset / M_WORLD_WIDTH) * M_TILE_ASPECT + M_TILE_ASPECT / 2);
	...
	{
		return{ ((float)in_x + .5f) * M_TILE_ASPECT, ((float)in_y + .5f) * M_TILE_ASPECT };
	}
		...
		m_mutable_t& out_mu)
	{
			...
			)
		{
			...
			if (INDEX == in_im.logic_indices[M_LOGIC_WALKER])
			{
				...
				mob.walker.speed = M_WALKER_SPEED;
		...
		m_mutable_t& out_mu)
	{
		...
		for (
			...
			out_mu.world_visited[i] = M_VISITED_NOT;
	...
	void m_back_to_checkpoint(m_mutable_t& out_mu, m_events_t& out_events)
	{
			...
			)
		{
			...
			if (M_VISITED == out_mu.world_visited[i])
				out_mu.world_visited[i] = M_VISITED_NOT;
		}
	...
	void m_load(m_immutable_t& out_im)
	{
		...
		for (uint32_t i = 0; i < 60000; ++i)
		{
			...
			const int32_t offset = x + y * M_WORLD_WIDTH;
			assert(offset >= 0);
	...
	bool m_mob_sees_hero(const m_immutable_t& in_im, const m_mutable_t& in_mu, const m_mob_t& in_mob)
	{
		...
		{
			if (
				(o + M_WORLD_WIDTH) == HERO_OFFSET ||
				o == HERO_OFFSET ||
				(o - M_WORLD_WIDTH) == HERO_OFFSET ||
				(o - M_WORLD_WIDTH * 2) == HERO_OFFSET
				)
			{
				return true;
			}
		...
		if (m_game_active(out_mu))
		{
			...
			if (m_character_alive(out_mu))
			{
				...
				if (m_logic_is(M_LOGIC_CHECKPOINT, T, in_im))
				{
					...
					//void m_checkpoint(m_mutable_t & model, m_events_t & events)
					{
							...
							)
						{
							...
							if (M_VISITED == out_mu.world_visited[i])
								out_mu.world_visited[i] = M_VISITED_CHECKPOINT;
						}
				...
				//void m_model_visit(m_mutable_t& out_mu, m_events_t& events)
				{
					...
					{
						uint8_t& visited = out_mu.world_visited[G.x + G.y * M_WORLD_WIDTH];
	...
	*/
	...
	uint8_t m_visited(const int32_t in_x, const int32_t in_y, const m_mutable_t& in_mu)
	{
		if (m_in_world(in_x, in_y))
			return in_mu.world_visited[in_x + in_y * M_WORLD_WIDTH];
