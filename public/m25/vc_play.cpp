...
namespace m25
{
		...
		m_mutable_t& out_mu, vc_fatpack_t& out_fatpack, uint32_t& out_paused)
	{
		...
		if (out_paused)
		{
			//pause prompts
			{
				...
				if (NINJA_VC_GUI_LEFT & vc_gui_big_text(in_micron, x = in_micron.canvas_width / 2, y = in_micron.canvas_height / 2 + NINJA_VC_SOFT_BUTTON_HEIGHT, MICRON_KEY_ESCAPE, "ESC Resume", out_fatpack))
				{
					out_paused ^= 1;
				}

				if (NINJA_VC_GUI_LEFT & vc_gui_big_text(in_micron, x, y += NINJA_VC_SOFT_BUTTON_HEIGHT, MICRON_KEY_RETURN, "ENTER Quit", out_fatpack))
				{
					out_paused ^= 1;
		...
		vc_fatpack_t& out_fatpack, micron_t& out_micron)
	{
		...
		c_vec2i_t camera;
		{
			...
			camera.x = HERO_WORLD.x - NINJA_VC_SCREEN_WIDTH / 2;
			camera.y = HERO_WORLD.y - NINJA_VC_SCREEN_HEIGHT / 2;
			...
			if (
				(1 & cambits) &&
				hero_local.y < NINJA_VC_SCREEN_HEIGHT / 2
				)
			{
				camera.y = HERO_WORLD.y;
				camera.y /= NINJA_VC_SCREEN_HEIGHT;
				camera.y *= NINJA_VC_SCREEN_HEIGHT;
			}
			...
			if (
				(4 & cambits) &&
				hero_local.x < NINJA_VC_SCREEN_WIDTH / 2
				)
			{
				camera.x = HERO_WORLD.x;
				camera.x /= NINJA_VC_SCREEN_WIDTH;
				camera.x *= NINJA_VC_SCREEN_WIDTH;
			}
			...
			if (camera.x < 0)
				camera.x = 0;
			else if (camera.x > (M_WORLD_WIDTH * M_TILE_ASPECT - NINJA_VC_SCREEN_WIDTH))
				camera.x = M_WORLD_WIDTH * M_TILE_ASPECT - NINJA_VC_SCREEN_WIDTH;

			if (camera.y < 0)
				camera.y = 0;
			else if (camera.y > (M_WORLD_HEIGHT * M_TILE_ASPECT - NINJA_VC_SCREEN_HEIGHT))
				camera.y = M_WORLD_HEIGHT * M_TILE_ASPECT - NINJA_VC_SCREEN_HEIGHT;
		}
					...
					)
				{
					...
					if (SCREEN_OFFSET < (NINJA_VC_SCREENS_IN_WORLD_X * NINJA_VC_SCREENS_IN_WORLD_Y))
					{
							...
							)
						{
							vc_canvas_clear(
								(1 & CAMBITS) ? ON : OFF,
								(x * M_TILE_ASPECT) - camera.x + 4,
								(y * M_TILE_ASPECT) - camera.y + 4,
