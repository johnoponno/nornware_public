...
namespace m25
{
	...
	{
		return
			in_mob.position.x > in_mu.hero_position.x - M_CHAR_HALF_WIDTH &&
			in_mob.position.x < in_mu.hero_position.x + M_CHAR_HALF_WIDTH &&
			in_mob.position.y > in_mu.hero_position.y - M_CHAR_HALF_HEIGHT &&
			in_mob.position.y < in_mu.hero_position.y + M_CHAR_HALF_HEIGHT;
	}
	...
	static uint32_t __bounds_tests(m_mutable_t& out_mu)
	{
		...
		else if (out_mu.hero_position.x >= M_WORLD_WIDTH * M_TILE_ASPECT)
		{
			out_mu.hero_speed.x = -out_mu.hero_speed.x;
			out_mu.hero_position.x = M_WORLD_WIDTH * M_TILE_ASPECT - 1;
		}
		...
		else if (out_mu.hero_position.y >= M_WORLD_HEIGHT * M_TILE_ASPECT)
		{
		...
		m_mutable_t& out_mu, m_events_t& out_events)
	{
		...
		if (__is_solid_below(in_im, out_mu))
		{
			if (out_mu.hero_air)
			{
				m_events_push(out_mu.hero_position + c_vec2f_t{ 0.f, (float)M_CHAR_HALF_HEIGHT }, M_NUM_COD, M_EVT_HERO_LANDED, out_events);
				out_mu.hero_air = false;
			}
			out_mu.hero_speed.y = 0.f;
			out_mu.hero_position.y = (float)((int32_t)out_mu.hero_position.y / M_TILE_ASPECT);
			out_mu.hero_position.y *= M_TILE_ASPECT;
			out_mu.hero_position.y += M_TILE_ASPECT - M_CHAR_HALF_HEIGHT;
		}
		else
		{
		...
		if (M_TYPE_SOLID == m_tile_type_at_position(m_character_test_above(out_mu), true, in_im, out_mu))
		{
			...
			out_mu.hero_position.y = (float)((int32_t)out_mu.hero_position.y / M_TILE_ASPECT);
			out_mu.hero_position.y *= M_TILE_ASPECT;
			out_mu.hero_position.y += M_CHAR_HALF_HEIGHT;
		}
		...
		if (out_mu.hero_speed.x < 0.f && __is_solid_left(in_im, out_mu))
		{
			...
			out_mu.hero_position.x = (float)((int32_t)out_mu.hero_position.x / M_TILE_ASPECT);
			out_mu.hero_position.x *= M_TILE_ASPECT;
			out_mu.hero_position.x += M_CHAR_HALF_WIDTH;
		}
		...
		if (out_mu.hero_speed.x > 0.f && m_character_is_solid_right(in_im, out_mu))
		{
			...
			out_mu.hero_position.x = (float)((int32_t)out_mu.hero_position.x / M_TILE_ASPECT);
			out_mu.hero_position.x *= M_TILE_ASPECT;
			out_mu.hero_position.x += M_TILE_ASPECT - M_CHAR_HALF_WIDTH;
		}
	}
	...
	{
		return M_FLAGS_LEFT & in_mu.hero_input;
	}

	static bool __right_input(const m_mutable_t& in_mu)
	{
		return M_FLAGS_RIGHT & in_mu.hero_input;
	}
	...
	static void __melee_mobs(
		...
		m_mutable_t& out_mu, m_events_t& out_events)
	{
		const float X_OFFSET = out_mu.hero_mirror ? 0.f : -M_MELEE_RANGE_X;
		const float PUSH = out_mu.hero_mirror ? HEAVY_PUSH : -HEAVY_PUSH;
		const c_vec2f_t MELEE_MIN{ out_mu.hero_position.x + X_OFFSET, out_mu.hero_position.y - M_MELEE_RANGE_Y };
		const c_vec2f_t MELEE_MAX{ out_mu.hero_position.x + X_OFFSET + M_MELEE_RANGE_X, out_mu.hero_position.y + M_MELEE_RANGE_Y };
	...
	{
		//switches
		if (__pressed_input(M_FLAGS_DOWN, out_mu))
		{
		...
		{
			if (M_FLAGS_SPRINT & out_mu.hero_input)
		...
		if (__pressed_input(M_FLAGS_JUMP, out_mu))	//not auto-jump
		{
			if (out_mu.hero_air)
			{
				...
				if (
					IN_LIQUID &&
					__pressed_input(M_FLAGS_JUMP, out_mu) &&
		...
		//melee
		if (__pressed_input(M_FLAGS_MELEE, out_mu))
		{
	...
	{
		c_vec2f_t p{ in_mu.hero_position.x - M_CHAR_HALF_WIDTH / 2, in_mu.hero_position.y + M_CHAR_HALF_HEIGHT };

		if (in_second)
			p.x += M_CHAR_HALF_WIDTH;
	...
	{
		return { in_mu.hero_position.x, in_mu.hero_position.y - M_CHAR_HALF_HEIGHT };
	}
		...
		m_mutable_t& out_mu)
	{
		...
		if (__pressed_input(M_FLAGS_JUMP, out_mu))
		{
	...
	{
		c_vec2f_t result = { in_mu.hero_position.x - M_CHAR_HALF_WIDTH, in_mu.hero_position.y - 8 };
		if (in_second)
			result.y += 16;
		return result;
	}
