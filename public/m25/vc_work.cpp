...
namespace m25
{
		...
		vc_fx_t& out_instance, vc_fatpack_t& out_fatpack, micron_t& out_micron)
	{
		...
		case VC_FX_TILE:
			...
			vc_octamap_blit_key_clip(
				C_ROUND(out_instance.position.x - in_camera.x - M_TILE_ASPECT / 2), C_ROUND(out_instance.position.y - in_camera.y - M_TILE_ASPECT / 2),
				M_TILE_ASPECT, M_TILE_ASPECT,
				(out_instance.tile.tile % in_tiles_on_source_x) * M_TILE_ASPECT,
				(out_instance.tile.tile / in_tiles_on_source_x) * M_TILE_ASPECT,
	...
	static bool __on_hero_screen(const c_vec2f_t& in_world, const m_mutable_t& in_mu)
	{
		...
		{
			(int32_t)in_world.x / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_X),
			(int32_t)in_world.y / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_Y)
		};
	...
	{
		switch (m_tile_type_at_position({ in_mu.hero_position.x, in_mu.hero_position.y + M_CHAR_HALF_HEIGHT + M_TILE_ASPECT * 2.f }, true, in_im, in_mu))
		{
		case M_TYPE_SOLID:
		case M_TYPE_PLATFORM:
			return true;
		...
		micron_t& out_micron)
	{
		...
		//FIXME: re-order the tiles in memory post-load so that each tile is cohesive memory (M_TILE_ASPECT x lots bitmap...) -> can write a more efficient blit
		...
		if (
			in_dst_x >= 0 &&
			in_dst_y >= 0 &&
			in_dst_x + M_TILE_ASPECT <= out_micron.canvas_width &&
			in_dst_y + M_TILE_ASPECT <= out_micron.canvas_height
			)
		{
			if (NINJA_VC_TILE_BITS_KEY & in_assets.tile_bits[in_tile_index])
				vc_octamap_blit_key(in_dst_x, in_dst_y, M_TILE_ASPECT, M_TILE_ASPECT, TILE_SRC.x, TILE_SRC.y, in_src, out_micron);
			else
				vc_octamap_blit(in_dst_x, in_dst_y, M_TILE_ASPECT, M_TILE_ASPECT, TILE_SRC.x, TILE_SRC.y, in_src, out_micron);
		}
		else
		{
			if (NINJA_VC_TILE_BITS_KEY & in_assets.tile_bits[in_tile_index])
				vc_octamap_blit_key_clip(in_dst_x, in_dst_y, M_TILE_ASPECT, M_TILE_ASPECT, TILE_SRC.x, TILE_SRC.y, in_src, out_micron);
			else
				vc_octamap_blit_clip(in_dst_x, in_dst_y, M_TILE_ASPECT, M_TILE_ASPECT, TILE_SRC.x, TILE_SRC.y, in_src, out_micron);
		}
#else
		vc_octamap_blit_key_clip(
			dst_x, dst_y,
	...
	static void __draw_terminal(const c_vec2i_t& aCamera, const uint32_t anOffset, const vc_octamap_t& bitmap, vc_canvas_t& canvas)
	{
		...
		w.y -= bitmap.header->image_spec_height + M_TILE_ASPECT;
		if (w.x < 0)
			w.x = 0;
	...
	c_vec2i_t vc_tile_src(const int32_t in_tiles_on_source_x, const int32_t in_tile_index)
	{
		return
		{
			(in_tile_index % in_tiles_on_source_x) * M_TILE_ASPECT,
				(in_tile_index / in_tiles_on_source_x) * M_TILE_ASPECT,
		};
	}
	...
	bool vc_view_init(vc_assets_t& out_assets, vc_fatpack_t& out_fatpack, micron_t& out_micron)
	{
		...
		/*
		out_fatpack.canvas = vc_canvas_make(NINJA_VC_SCREEN_WIDTH, NINJA_VC_SCREEN_HEIGHT);
	...
	c_vec2i_t vc_hero_screen(const m_mutable_t& in_mu)
	{
		...
		p.x /= NINJA_VC_SCREEN_WIDTH;
		p.y /= NINJA_VC_SCREEN_HEIGHT;
		...
		vc_fatpack_t& out_fatpack, micron_t& out_micron)
	{
		if (m_character_alive(in_mu))
		{
			...
			{
				if (
					(in_mu.hero_input & M_FLAGS_LEFT || in_mu.hero_input & M_FLAGS_RIGHT) &&
					in_mu.hero_speed.x
					)
				{
					...
					else if (M_FLAGS_SPRINT & in_mu.hero_input)
			...
			if (melee < 8)
			{
				const int32_t RANGE = M_MELEE_RANGE_X - 16;	//adjust this so it looks right
				...
				if (melee < 4)
				{
					vc_canvas_line(SCREEN.x + (melee * STEP) * d, SCREEN.y - M_MELEE_RANGE_Y, SCREEN.x + RANGE * d, SCREEN.y - M_MELEE_RANGE_Y + melee * 8, 0, out_micron);
				}
				else
				{
					melee -= 4;
					vc_canvas_line(SCREEN.x + (RANGE - melee * STEP) * d, SCREEN.y + M_MELEE_RANGE_Y, SCREEN.x + RANGE * d, SCREEN.y - M_MELEE_RANGE_Y + melee * 8, 0, out_micron);
				}
			}
		}
	}
	...
	void vc_draw_plax(
		...
		c_xorshift128_t& out_prng, micron_t& out_micron)
	{
		const int32_t SCREEN_X = in_camera.x / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_X);
		const int32_t SCREEN_Y = in_camera.y / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_Y);
	...
	*/
	...
	{
		return FS_TGA_HEADER(in_tiles)->image_spec_width / M_TILE_ASPECT;
	}
		...
		vc_fatpack_t& out_fatpack, micron_t& out_micron)
	{
		...
		//calc starting grid
		const c_vec2i_t GRID{ in_camera.x / M_TILE_ASPECT, in_camera.y / M_TILE_ASPECT };

		//calc subtile shift
		const c_vec2i_t SHIFT{ in_camera.x % M_TILE_ASPECT, in_camera.y % M_TILE_ASPECT };
				...
				if (WANG_2_CORNER_SYSTEM.bits)
				{
						...
						)
					{
						if ((1 << i) & WANG_2_CORNER_SYSTEM.bits)
						{
							...
							assert(DRAW_INDEX >= 0 && DRAW_INDEX < M_MAX_TILE);

							__draw_tile(
								in_assets,
								TILES_ON_SOURCE_X,
								M_TILE_ASPECT / 2 + x * M_TILE_ASPECT - SHIFT.x,
								M_TILE_ASPECT / 2 + y * M_TILE_ASPECT - SHIFT.y,
					...
					)
				{
					...
					if (in_flags & NINJA_VC_TILEFLAGS_REPLACE)
					{
						...
						else if (m_logic_is(M_LOGIC_RED, TILE, in_im))
						{
							if (in_mu.tri != 0)
								...
								mu.rgb_glows.push_back({ 0, x * M_TILE_ASPECT - shift.x, y * M_TILE_ASPECT - shift.y });
#endif
						}
						else if (m_logic_is(M_LOGIC_GREEN, TILE, in_im))
						{
							if (in_mu.tri != 1)
								...
								mu.rgb_glows.push_back({ 1, x * M_TILE_ASPECT - shift.x, y * M_TILE_ASPECT - shift.y });
#endif
						}
						else if (m_logic_is(M_LOGIC_BLUE, TILE, in_im))
						{
							if (in_mu.tri != 2)
								...
								mu.rgb_glows.push_back({ 2, x * M_TILE_ASPECT - shift.x, y * M_TILE_ASPECT - shift.y });
#endif
						}
						//taken flowers
						else if (m_logic_is(M_LOGIC_FLOWER, TILE, in_im))
						{
							if (VISITED != M_VISITED_NOT)
								index = (uint16_t)vc_gfx_index_of(VC_GFX_CLEAR, in_assets);
						}
						//diverse -> clear
						else if (
						...
						{
							if ((uint32_t)(GRID.x + x + (GRID.y + y) * M_WORLD_WIDTH) == in_mu.checkpoint_offset)
					...
					if (WANG_BLOB == in_assets.tile_info[index].wang)
					{
						...
						__draw_tile(
							in_assets,
							TILES_ON_SOURCE_X,
							x * M_TILE_ASPECT - SHIFT.x,
							y * M_TILE_ASPECT - SHIFT.y,
	...
	*/
	...
	{
		return __screen({ in_world.x / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_X), in_world.y / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_Y) }, in_assets);
	}
	...
	{
		return vc_screen_cambits({ in_world.x / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_X), in_world.y / (M_TILE_ASPECT * NINJA_VC_TILES_ON_SCREEN_Y) }, in_assets);
	}
