	...
	w32_d3d9_softdraw_adapter_t& in_adapter)
{
	...
	if (!in_adapter.texture || in_adapter.texture_aspect < (uint32_t)in_canvas.width || in_adapter.texture_aspect < (uint32_t)in_canvas.height)
	{
		...
		if (in_adapter.ALPHA)
		{
			VERIFY(w32_d3d9_state.m_d3d_device->CreateTexture(in_adapter.texture_aspect, in_adapter.texture_aspect, 1, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED, &in_adapter.texture, nullptr));
		}
		else
		{
			VERIFY(w32_d3d9_state.m_d3d_device->CreateTexture(in_adapter.texture_aspect, in_adapter.texture_aspect, 1, 0, D3DFMT_R5G6B5, D3DPOOL_MANAGED, &in_adapter.texture, nullptr));
		}
	}
...

	{
		...
		::IDirect3DStateBlock9* sb = w32_d3d9_state_block_begin(in_mode, ::D3DCULL_CCW, D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1, in_filter, in_adapter.texture);
		...
		//render in_adapter quad
		{
			...
			VERIFY(w32_d3d9_state.m_d3d_device->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, 4, 2, INDICES, D3DFMT_INDEX16, VERTICES, sizeof(VERTICES[0])));
		}
