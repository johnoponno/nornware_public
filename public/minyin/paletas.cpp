	...
	paletas_t& out_paletas, pixel_t* out_palette)
{
	...
	::memset(out_palette, 0, sizeof(pixel_t) * 256);
	{
		...
		std::vector<std::vector<pixel_t>> all_buckets;
		{
			...
			{
				for (
					pixel_t* pixel = (pixel_t*)FS_TGA_PIXELS(SOURCE_IMAGE);
					pixel < (pixel_t*)FS_TGA_PIXELS(SOURCE_IMAGE) + FS_TGA_HEADER(SOURCE_IMAGE)->image_spec_width * FS_TGA_HEADER(SOURCE_IMAGE)->image_spec_height;
					++pixel
					)
				{
					++total_source_pixels;
	...
	//create all the palettize bitmaps by remapping all pixels to the final palette of averaged colors (use the nearest color)
	{
		...
		for (uint32_t i = 0; i < out_paletas.items.size(); ++i)
		{
			...
			for (int32_t y = 0; y < FS_TGA_HEADER(source_images[i])->image_spec_height; ++y)
			{
				...
				{
					const pixel_t* SOURCE_IMAGE_I_PIXEL = (pixel_t*)FS_TGA_PIXELS(source_images[i]) + x + y * FS_TGA_HEADER(source_images[i])->image_spec_width;
