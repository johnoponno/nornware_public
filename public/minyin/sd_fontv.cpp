...
sd_fontv_t::sd_fontv_t(const int32_t aCharSpacing)
{
...
void sd_fontv_print(const sd_fontv_t& FONT, const sd_fontv_blit_mode_t aMode, const int32_t aDstX, const int32_t aDstY, const char* message, sd_bitmap_t& aDst)
{
	...
	for (int32_t i = 0; i < length; ++i)
	{
		...
		//printable chars
		else if (ch >= SD_FONTV_CHARBEGIN && ch <= SD_FONTV_CHAREND)
		{
			ch -= SD_FONTV_CHARBEGIN;

			switch (aMode)
			{
			case sd_fontv_blit_mode_t::NORMAL:
	...
	while (index < length)
	{
		...
		else
		{
			...
			if (chr == ' ')
				...
				result += f.characters[chr - SD_FONTV_CHARBEGIN].w + f.char_spacing;
		}
		++index;
	}
...
{
	sd_fontv_print(f, sd_fontv_blit_mode_t::KEY, aX, aY, aText, aCanvas);
}

bool sd_fontv_load_24(sd_fontv_t& font, const char* aFileName, const bool aBlackKeyFlag)
{
	...
	font.space_width = font.characters['_' - SD_FONTV_CHARBEGIN].w;
