...
namespace mlm
{
		...
		d_dev_t& out_dev)
	{
		...
		if (in_shrink)
			out_dev._canvas16.rect(dst.x + 1, dst.y + 1, dst.x + MLM_M_TILE_ASPECT - 2, dst.y + MLM_M_TILE_ASPECT - 2, in_color);
		else
			out_dev._canvas16.rect(dst.x, dst.y, dst.x + MLM_M_TILE_ASPECT - 1, dst.y + MLM_M_TILE_ASPECT - 1, in_color);
	}
#endif
		...
		sd_bitmap_t& out_canvas)
	{
			...
			)
		{
			for (
				int32_t x = 0;
				x < FS_TGA_HEADER(in_tiles)->image_spec_width / MLM_M_TILE_ASPECT;
				++x
				)
			{
				const int32_t TILE = x + y * (FS_TGA_HEADER(in_tiles)->image_spec_width / MLM_M_TILE_ASPECT);
		...
		sd_bitmap_t& out_canvas)
	{
		...
		if (in_edge_flag)
		{
			out_canvas.add_h_line(0, MLM_M_TILE_ASPECT, mp.y, SD_DARK_GRAY);
			out_canvas.add_h_line(0, MLM_M_TILE_ASPECT, mp.y + MLM_M_TILE_ASPECT * in_size - 1, SD_DARK_GRAY);
			out_canvas.add_h_line(out_micron.canvas_width - MLM_M_TILE_ASPECT, out_micron.canvas_width, mp.y, SD_DARK_GRAY);
			out_canvas.add_h_line(out_micron.canvas_width - MLM_M_TILE_ASPECT, out_micron.canvas_width, mp.y + MLM_M_TILE_ASPECT * in_size - 1, SD_DARK_GRAY);

			out_canvas.add_v_line(mp.x, 0, MLM_M_TILE_ASPECT, SD_DARK_GRAY);
			out_canvas.add_v_line(mp.x + MLM_M_TILE_ASPECT * in_size - 1, 0, MLM_M_TILE_ASPECT, SD_DARK_GRAY);
			out_canvas.add_v_line(mp.x, out_micron.canvas_height - MLM_M_TILE_ASPECT, out_micron.canvas_height, SD_DARK_GRAY);
			out_canvas.add_v_line(mp.x + MLM_M_TILE_ASPECT * in_size - 1, out_micron.canvas_height - MLM_M_TILE_ASPECT, out_micron.canvas_height, SD_DARK_GRAY);
		}

		out_canvas.clear(nullptr, sd_operation_t::ADD, SD_GRAY, mp.x, mp.y, MLM_M_TILE_ASPECT, MLM_M_TILE_ASPECT);
	}
#endif
		...
		d_dev_t& out_dev)
	{
		...
		for (
			...
			out_dev._canvas16.add_h_line(0, MLM_M_WORLD_WIDTH, i * NINJA_VC_TILES_ON_SCREEN_Y, SD_DARK_GRAY);
		...
		for (
			...
			out_dev._canvas16.add_v_line(i * NINJA_VC_TILES_ON_SCREEN_X, 0, MLM_M_WORLD_HEIGHT, SD_DARK_GRAY);
	}
#else
		...
		case d_mode_t::TERMINAL:
				...
				)
			{
				...
				sd_bitmap_clear(
					...
					sd_operation_t::MUL,
					SD_GREEN,
					SP.x - MLM_M_TILE_ASPECT / 2 + 4, SP.y - MLM_M_TILE_ASPECT / 2 + 4,
