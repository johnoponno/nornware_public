...
namespace mlm
{
	...
	{
		_strings.resize(_strings.size() + 1);
		string_t& g = _strings[_strings.size() - 1];
	...
	c_vec2i_t d_dev_t::render_size(const int32_t in_backbuffer_width, const int32_t in_backbuffer_height) const
	{
		if (_bit_maximize_size)
		{
			...
			{
				if (
					MLM_VC_SCREEN_WIDTH * mul < in_backbuffer_width &&
					MLM_VC_SCREEN_HEIGHT * mul < in_backbuffer_height
					)
				{
					++mul;
				}
				else
				{
					...
					return { MLM_VC_SCREEN_WIDTH * mul, MLM_VC_SCREEN_HEIGHT * mul };
				}
			}
		}

		return{ MLM_VC_SCREEN_WIDTH, MLM_VC_SCREEN_HEIGHT };
	}
